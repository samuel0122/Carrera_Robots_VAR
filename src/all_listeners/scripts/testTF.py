...
print('Mutating...')
shape = [weight.shape for weight in weights]
print(shape)

weights = [np.asarray([w + np.random.uniform(-mutation_range, mutation_range, size=w.shape) if np.random.uniform() < mutation_rate else w for w in weight]) for weight in weights]
shape = [weight.shape for weight in weights]
print(shape)
# Generate a new child by mutating the model's parameters
